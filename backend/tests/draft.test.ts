...
describe('Draft Generator', () => {
  test('buildPrompt creates valid prompt', () => {
    ...
    expect(prompt).toContain('dinner plan for 5 nights');
  ...
  test('generateDraft returns valid draft (calls OpenAI API)', async () => {
    console.log('\n--- Calling OpenAI API ---');
  ...
  test('generateDraft respects dietary filters', async () => {
    const glutenFreeRequest: DraftRequest = {
      ...testRequest,
      dietaryFilters: {
        glutenFree: true,
        dairyFree: false,
    ...
    const draft = await generateDraft(glutenFreeRequest, { verbose: false });

    // Verify we got a draft
    expect(draft.meals.length).toBe(glutenFreeRequest.dinnerCount);

    // All meals should be from gluten-free pool (verify by checking IDs exist in filtered list)
    const glutenFreeMeals = getMeals(glutenFreeRequest.dietaryFilters);
    const validIds = new Set(glutenFreeMeals.map((m) => m.id));

    for (const meal of draft.meals) {
      if (meal.mealId !== null) {
        expect(validIds.has(meal.mealId)).toBe(true);
      }
    }

    console.log(`Gluten-free draft generated with ${draft.meals.length} meals`);
  });
